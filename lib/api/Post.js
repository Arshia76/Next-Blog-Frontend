...
export const getAllPosts = async (
  ...
  search,
  page = 1,
  take = process.env.NEXT_PUBLIC_TAKE
) => {
  if (
    ...
    const { data } = await axios.get(
      `${URL}/posts?category=${category}&search=${search}&take=${take}&page=${page}`,
      {
        headers: {
          'accept-encoding': '*',
        },
    ...
    const { data } = await axios.get(`${URL}/posts?take=${take}&page=${page}`, {
      headers: {
        'accept-encoding': '*',
      },
    });
